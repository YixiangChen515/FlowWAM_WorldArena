...
def generation_loop(runner, video_path='./test_videos', output_dir='./results', batch_size=1, cfg_scale=1.0, cfg_rescale=0.0, sample_steps=1, seed=666, res_h=1280, res_w=720, sp_size=1, out_fps=None):
    ...
    # generation loop
    _rank = get_data_parallel_rank()
    for videos, text_embeds in tqdm(
        zip(original_videos_local, positive_prompts_embeds),
        total=len(original_videos_local),
        desc=f"rank{_rank}",
        position=_rank,
    ):
        # resume: skip if all outputs already exist
        if all(os.path.exists(os.path.join(tgt_path, os.path.basename(v))) for v in videos):
            continue
            ...
            for path, input, sample, ori_length, save_fps in zip(
                ...
                if use_colorfix:
                    ...
                    sample = wavelet_reconstruction(
                        sample, input[: sample.size(0)].to(_dev)
                    ).to("cpu")
                else:
